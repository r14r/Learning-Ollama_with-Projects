...
Fokussiere dich auf: complete_text, complete_story.

```python
def complete_text(prompt, model, max_tokens):
    return "todo"
```

Schritt 4: Kernlogik erweitern
Fokussiere dich auf: complete_code, main.

```python
def complete_code(code_snippet, model):
    return "todo"
```
